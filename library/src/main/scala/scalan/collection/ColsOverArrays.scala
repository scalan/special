package scalan.collection {
  ...
  trait ColsOverArrays extends Base { self: Library =>
    ...
    abstract class ReplCol[A](val value: Rep[A], val length: Rep[Int]) extends Col[A] {
      ...
      def map[B](f: Rep[scala.Function1[A, B]]): Rep[Col[B]] = ReplCol(f.apply(ReplCol.this.value), ReplCol.this.length);
      def foreach(f: Rep[scala.Function1[A, Unit]]): Rep[Unit] = Predef.???;
      ...
      def fold[B](zero: Rep[B])(op: Rep[scala.Function1[scala.Tuple2[B, A], B]]): Rep[B] = Predef.???;
