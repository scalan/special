...
object CSizePrim extends EntityObject("CSizePrim") {
  ...
  case class CSizePrimCtor[Val]
      (override val dataSize: Rep[Long])(implicit val eVal: Elem[Val])
    extends CSizePrim[Val](dataSize) with Def[CSizePrim[Val]] {
    lazy val selfType = element[CSizePrim[Val]]
    override def transform(t: Transformer) = CSizePrimCtor[Val](t(dataSize))(eVal)
  }
  ...
  class CSizePrimElem[Val](val iso: Iso[CSizePrimData[Val], CSizePrim[Val]])(implicit override val eVal: Elem[Val])
    ...
    override def convertSizePrim(x: Rep[SizePrim[Val]]) = RCSizePrim(x.dataSize)
    override def getDefaultRep = RCSizePrim(0l)
  ...
  class CSizePrimIso[Val](implicit eVal: Elem[Val])
    ...
    private lazy val _safeFrom = fun { p: Rep[CSizePrim[Val]] => p.dataSize }
    override def from(p: Rep[CSizePrim[Val]]) =
      tryConvert[CSizePrim[Val], Long](eTo, eFrom, p, _safeFrom)
  ...
  class CSizePrimCompanionCtor extends CompanionDef[CSizePrimCompanionCtor] with CSizePrimCompanion {
    ...
    def apply[Val](dataSize: Rep[Long])(implicit eVal: Elem[Val]): Rep[CSizePrim[Val]] =
      mkCSizePrim(dataSize)
  ...
  implicit class ExtendedCSizePrim[Val](p: Rep[CSizePrim[Val]])(implicit eVal: Elem[Val]) {
  ...
  def mkCSizePrim[Val]
    (dataSize: Rep[Long])(implicit eVal: Elem[Val]): Rep[CSizePrim[Val]] = {
    new CSizePrimCtor[Val](dataSize)
  }
  def unmkCSizePrim[Val](p: Rep[SizePrim[Val]]) = p.elem.asInstanceOf[Elem[_]] match {
    case _: CSizePrimElem[Val] @unchecked =>
      Some((asRep[CSizePrim[Val]](p).dataSize))
    case _ =>
      None
  }
...
object CSizeFunc extends EntityObject("CSizeFunc") {
  case class CSizeFuncCtor[Env, Arg, Res]
      (override val sizeEnv: Rep[Size[Env]], override val sizeFunc: Rep[Long])(implicit val eArg: Elem[Arg], val eRes: Elem[Res])
    extends CSizeFunc[Env, Arg, Res](sizeEnv, sizeFunc) with Def[CSizeFunc[Env, Arg, Res]] {
    ...
    override def transform(t: Transformer) = CSizeFuncCtor[Env, Arg, Res](t(sizeEnv), t(sizeFunc))(eArg, eRes)
...
!!!("Cannot convert from SizeFunc to CSizeFunc: missing fields List(sizeFunc)")
    override def getDefaultRep = RCSizeFunc(element[Size[Env]].defaultRepValue, 0l)
    override lazy val tag = {
  ...
  // state representation type
  type CSizeFuncData[Env, Arg, Res] = (Size[Env], Long)
  ...
  class CSizeFuncIso[Env, Arg, Res](implicit eEnv: Elem[Env], eArg: Elem[Arg], eRes: Elem[Res])
    ...
    private lazy val _safeFrom = fun { p: Rep[CSizeFunc[Env, Arg, Res]] => (p.sizeEnv, p.sizeFunc) }
    override def from(p: Rep[CSizeFunc[Env, Arg, Res]]) =
      tryConvert[CSizeFunc[Env, Arg, Res], (Size[Env], Long)](eTo, eFrom, p, _safeFrom)
    override def to(p: Rep[(Size[Env], Long)]) = {
      val Pair(sizeEnv, sizeFunc) = p
      RCSizeFunc(sizeEnv, sizeFunc)
    }
    lazy val eFrom = pairElement(element[Size[Env]], element[Long])
  ...
  class CSizeFuncCompanionCtor extends CompanionDef[CSizeFuncCompanionCtor] with CSizeFuncCompanion {
    ...
    @scalan.OverloadId("fromData")
    def apply[Env, Arg, Res](p: Rep[CSizeFuncData[Env, Arg, Res]])(implicit eArg: Elem[Arg], eRes: Elem[Res]): Rep[CSizeFunc[Env, Arg, Res]] = {
      implicit val eEnv = p._1.eVal
      isoCSizeFunc[Env, Arg, Res].to(p)
    }

    @scalan.OverloadId("fromFields")
    def apply[Env, Arg, Res](sizeEnv: Rep[Size[Env]], sizeFunc: Rep[Long])(implicit eArg: Elem[Arg], eRes: Elem[Res]): Rep[CSizeFunc[Env, Arg, Res]] =
      mkCSizeFunc(sizeEnv, sizeFunc)
  ...
  implicit class ExtendedCSizeFunc[Env, Arg, Res](p: Rep[CSizeFunc[Env, Arg, Res]])(implicit eArg: Elem[Arg], eRes: Elem[Res]) {
  ...
  def mkCSizeFunc[Env, Arg, Res]
    (sizeEnv: Rep[Size[Env]], sizeFunc: Rep[Long])(implicit eArg: Elem[Arg], eRes: Elem[Res]): Rep[CSizeFunc[Env, Arg, Res]] = {
    new CSizeFuncCtor[Env, Arg, Res](sizeEnv, sizeFunc)
  }
  def unmkCSizeFunc[Env, Arg, Res](p: Rep[SizeFunc[Env, Arg, Res]]) = p.elem.asInstanceOf[Elem[_]] match {
    case _: CSizeFuncElem[Env, Arg, Res] @unchecked =>
      Some((asRep[CSizeFunc[Env, Arg, Res]](p).sizeEnv, asRep[CSizeFunc[Env, Arg, Res]](p).sizeFunc))
    case _ =>
      None
  }
