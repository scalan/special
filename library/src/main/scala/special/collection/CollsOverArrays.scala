package special.collection {
  ...
  trait CollsOverArrays extends Base { self: Library =>
    ...
    abstract class CollOverArray[A](val arr: Rep[WArray[A]]) extends Coll[A] {
      ...
      def zip[B](ys: Rep[Coll[B]]): Rep[PairColl[A, B]] = CollOverArray.this.builder.pairCol[A, B](this, ys);
      @NeverInline def append(other: Rep[Coll[A]]): Rep[Coll[A]] = delayInvoke
    };
    abstract class ColOverArrayBuilder extends ColBuilder {
      def pairCol[A, B](as: Rep[Coll[A]], bs: Rep[Coll[B]]): Rep[PairColl[A, B]] = RPairOfCols(as, bs);
    ...
    abstract class PairOfCols[L, R](val ls: Rep[Coll[L]], val rs: Rep[Coll[R]]) extends PairColl[L, R] {
      ...
      override def slice(from: Rep[Int], until: Rep[Int]): Rep[PairColl[L, R]] = PairOfCols.this.builder.pairCol[L, R](PairOfCols.this.ls.slice(from, until), PairOfCols.this.rs.slice(from, until));
      ...
      def zip[B](ys: Rep[Coll[B]]): Rep[PairColl[scala.Tuple2[L, R], B]] = PairOfCols.this.builder.pairCol[scala.Tuple2[L, R], B](this, ys)
    };
    abstract class CReplColl[A](val value: Rep[A], val length: Rep[Int]) extends ReplColl[A] {
      ...
      @NeverInline def fold[B](zero: Rep[B], op: Rep[scala.Function1[scala.Tuple2[B, A], B]]): Rep[B] = delayInvoke;
      def zip[B](ys: Rep[Coll[B]]): Rep[PairColl[A, B]] = CReplColl.this.builder.pairCol[A, B](this, ys);
