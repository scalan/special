...
object MT0 extends EntityObject("MT0") {
  ...
  lazy val MT0Rep: Ref[MT0CompanionCtor] = new MT0CompanionCtor
...
!!!("Cannot convert from MetaTest to MT1: missing fields List(data)")
  ...
  class MT1Iso[T](implicit eT: Elem[T])
    ...
    private lazy val _safeFrom = fun { p: Ref[MT1[T]] => (p.data, p.size) }
    override def from(p: Ref[MT1[T]]) =
      tryConvert[MT1[T], (T, Int)](eTo, eFrom, p, _safeFrom)
    override def to(p: Ref[(T, Int)]) = {
  ...
  lazy val MT1Rep: Ref[MT1CompanionCtor] = new MT1CompanionCtor
...
implicit val eB = p._2.elem
  ...
  lazy val MT2Rep: Ref[MT2CompanionCtor] = new MT2CompanionCtor
