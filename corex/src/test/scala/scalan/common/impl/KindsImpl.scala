...
implicit lazy val eA = source.elem.typeArgs("A")._1.asElem[A]
  ...
  class KindElem[F[_], A, To <: Kind[F, A]](implicit _cF: Cont[F], _eA: Elem[A])
    ...
    override def convert(x: Ref[Def[_]]) = {
      val conv = fun {x: Ref[Kind[F, A]] => convertKind(x) }
      tryConvert(element[Kind[F, A]], this, x, conv)
    }

    def convertKind(x: Ref[Kind[F, A]]): Ref[To] = {
  ...
  object KindMethods {
    ...
    object mapBy {
      def unapply(d: Def[_]): Nullable[(Ref[Kind[F, A]], Ref[A => B]) forSome {type F[_]; type A; type B}] = d match {
...
object Return extends EntityObject("Return") {
  ...
  (override val a: Ref[A])(implicit cF: Cont[F])
  ...
  class ReturnElem[F[_], A](val iso: Iso[ReturnData[F, A], Return[F, A]])(implicit override val cF: Cont[F], override val eA: Elem[A])
    ...
    override def buildTypeArgs = super.buildTypeArgs ++ TypeArgs("F" -> (cF -> scalan.util.Invariant), "A" -> (eA -> scalan.util.Invariant))
    override def convertKind(x: Ref[Kind[F, A]]) = // Converter is not generated by meta
!!!("Cannot convert from Kind to Return: missing fields List(a)")
  ...
  class ReturnIso[F[_], A](implicit cF: Cont[F], eA: Elem[A])
    ...
    private lazy val _safeFrom = fun { p: Ref[Return[F, A]] => p.a }
    override def from(p: Ref[Return[F, A]]) =
      tryConvert[Return[F, A], A](eTo, eFrom, p, _safeFrom)
    override def to(p: Ref[A]) = {
      val a = p
      RReturn(a)
    }
  ...
  class ReturnCompanionCtor extends CompanionDef[ReturnCompanionCtor] with ReturnCompanion {
    ...
    def apply[F[_], A](a: Ref[A])(implicit cF: Cont[F]): Ref[Return[F, A]] =
      mkReturn(a)

    def unapply[F[_], A](p: Ref[Kind[F, A]]) = unmkReturn(p)
  }
  lazy val ReturnRep: Ref[ReturnCompanionCtor] = new ReturnCompanionCtor
  ...
  def mkReturn[F[_], A]
      (a: Ref[A])(implicit cF: Cont[F]): Ref[Return[F, A]] = {
    new ReturnCtor[F, A](a)
  }
  def unmkReturn[F[_], A](p: Ref[Kind[F, A]]) = p.elem.asInstanceOf[Elem[_]] match {
...
object Bind extends EntityObject("Bind") {
  case class BindCtor[F[_], S, B]
      (override val a: Ref[Kind[F, S]], override val f: Ref[S => Kind[F, B]])
...
implicit lazy val eB = f.elem.eRange.typeArgs("A")._1.asElem[B]
  ...
  class BindElem[F[_], S, B](val iso: Iso[BindData[F, S, B], Bind[F, S, B]])(implicit override val cF: Cont[F], val eS: Elem[S], val eB: Elem[B])
    ...
    override def convertKind(x: Ref[Kind[F, B]]) = // Converter is not generated by meta
!!!("Cannot convert from Kind to Bind: missing fields List(a, f)")
  ...
  class BindIso[F[_], S, B](implicit cF: Cont[F], eS: Elem[S], eB: Elem[B])
    ...
    private lazy val _safeFrom = fun { p: Ref[Bind[F, S, B]] => (p.a, p.f) }
    override def from(p: Ref[Bind[F, S, B]]) =
      tryConvert[Bind[F, S, B], (Kind[F, S], S => Kind[F, B])](eTo, eFrom, p, _safeFrom)
    override def to(p: Ref[(Kind[F, S], S => Kind[F, B])]) = {
      val Pair(a, f) = p
      RBind(a, f)
    }
...
implicit val eB = p._2.elem.eRange.typeArgs("A")._1.asElem[B]
    ...
    @scalan.OverloadId("fromFields")
    def apply[F[_], S, B](a: Ref[Kind[F, S]], f: Ref[S => Kind[F, B]]): Ref[Bind[F, S, B]] =
      mkBind(a, f)

    def unapply[F[_], S, B](p: Ref[Kind[F, B]]) = unmkBind(p)
  }
  lazy val BindRep: Ref[BindCompanionCtor] = new BindCompanionCtor
...
implicit val eB = p.f.elem.eRange.typeArgs("A")._1.asElem[B]
  ...
  def mkBind[F[_], S, B]
    (a: Ref[Kind[F, S]], f: Ref[S => Kind[F, B]]): Ref[Bind[F, S, B]] = {
    new BindCtor[F, S, B](a, f)
  }
  def unmkBind[F[_], S, B](p: Ref[Kind[F, B]]) = p.elem.asInstanceOf[Elem[_]] match {
