...
/**
  * The code is borrowed from https://github.com/mpilquist/simulacrum/blob/master/core/src/main/scala/simulacrum/typeclass.scala.
  * We try to keep it as close as possible to the original implementation
  * Difference summary:
  * 1) Replace macro annotations with Scalanizer annotations, available on later compilation phases (after typer)
  * 2) Changes in generated code to support requirements of SUnitDef
  */
