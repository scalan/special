...
class MonoidBuilderInst extends MonoidBuilder {
  ...
  override def pairMonoid[@sp(Int, Long) A, @sp(Int, Long) B](m1: Monoid[A], m2: Monoid[B]): Monoid[(A, B)] =
...
class PairMonoid[@sp(Int, Long) A, @sp(Int, Long) B](val m1: Monoid[A], m2: Monoid[B]) extends Monoid[(A,B)] {
