...
trait CommonExamples extends Scalan with SegmentsModule {
  ...
  lazy val t7 = fun { (in: Rep[IntervalData]) =>
    val Pair(s, l) = in
    val res = IFF (s < 0) THEN { RInterval(in):RSeg } ELSE { RSlice(0, l):RSeg }
    res.length
  }
  lazy val t8 = fun { (in: Rep[IntervalData]) =>
    val Pair(s, l) = in
    val Pair(i, res) = IFF (s < 0) THEN { Pair(1, RInterval(in):RSeg) } ELSE { Pair(2, RSlice(0, l):RSeg) }
    i + res.length
  }
  lazy val t9 = fun { (in: Rep[IntervalData]) =>
    val Pair(s, l) = in
    val segm = IFF (s < 0) THEN { RInterval(in):RSeg } ELSE { RSlice(0, l):RSeg }
    val res = IFF (l > 10) THEN { segm.shift(1) } ELSE { RSlice(0, l):RSeg }
    res.length
  }
  lazy val t10 = fun { (in: Rep[IntervalData]) =>
    val Pair(s, l) = in
    val res = IFF (s < 0) THEN { (RInterval(in): RSeg).asRight[Unit] } ELSE { (RSlice(0, l): RSeg).asRight[Unit] }
    res.fold(_ => 0, s => s.length)
  }
  lazy val t10_1 = fun { (in: Rep[IntervalData]) =>
    val Pair(s, l) = in
    val res = IFF (s < 0) THEN { (RInterval(in): RSeg).asLeft[Unit] } ELSE { (RSlice(0, l): RSeg).asLeft[Unit] }
    res.fold(s => s.length, _ => 0)
  }
  lazy val t10_2 = fun { (in: Rep[IntervalData]) =>
    val Pair(s, l) = in
    val res = IFF (s < 0) THEN {
